...
export class MainPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, private moltServ: MoltinService,
    ...
    loading.present();
    let tempProducts = [];
    this.moltServ.getMoltin().Products.All().then((products) => {
      products.data.forEach((product) => {
        let newProd = new Product(product.name, product.description, product.id, product.meta.timestamps.created_at,
      ...
      let promises = pics.map((pic) => {
        return this.moltServ.getMoltin().Files.Get(pic);
      });
      ...
      this.products.forEach((product) => {
        itemsProcessed += 1;
        res.forEach((pic) => {
          if (product.pic == pic.data.id) {
            product.pic = pic.data.link.href;
          }
        })
